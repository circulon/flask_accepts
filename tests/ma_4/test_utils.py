...
def test_get_default_model_name_default_names():
    from src.flask_accepts.utils.utils_ma_4 import _num_default_v4_models

    for model_num in range(5):
        result = utils.get_default_model_name()
        expected = f"DefaultResponseModel_{model_num + _num_default_v4_models}"
        assert result == expected
...
def test_ma_field_to_reqparse_argument_single_values():
    ...
    result = utils.ma_field_to_reqparse_argument(ma.Email(required=True))
